...
// }
const api_key = 'RGAPI-cfc42fd1-d6af-4244-9920-99f826ee64f3'
...
function search_rival_in_matches(rival_id, match_body, ret){
  ...
  for (var i = 0; i < participants.length; i++){
    if (participants[i]['player']['currentAccountId'] == rival_id){     
      for(var j = 0; j < 10; j++ ){  
        var champID = JSON.parse(match_body)['participants'][j]['championId'];        
...
// }
  ...
  .then(function(values){
    ...
    res.render('info', { username: username, rival: rival_username, match: all_matches, number: all_matches.length })    
  })
  .catch(console.warn);
